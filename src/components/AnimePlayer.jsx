    ...
    <div className="controls-row">
      ...
      >
        ...
        <div
          ...
          style={{ left: `${(current / (duration || 1)) * 100}%` }}
        ></div>
        {progressTooltip && <span className="tooltip" style={{ left: progressTooltip.x }}>{progressTooltip.time}</span>}
      </div>
      <div className="controls-main">
        <div className="left-controls">
          <button className="ctrl" onClick={() => skip(-10)} title="−10s">
        ...
        <div className="right-controls">
          <button className="ctrl" onClick={togglePiP} title="Picture-in-Picture">
            <span className="material-icons">picture_in_picture_alt</span>
          </button>
          <button className="ctrl" onClick={toggleFullscreen} title="Fullscreen">
...
export default function AnimePlayer({ url, poster, initialMuted = false }) {
  ...
  const skip = useCallback((s) => {
    const v = videoRef.current;
    if (v) {
      v.currentTime = Math.max(0, Math.min(v.duration || 1, v.currentTime + s));
    }
  }, []);
  ...
  const onProgressClick = useCallback(
    (e) => {
      ...
      const v = videoRef.current;
      v.currentTime = perc * (v.duration || 1);
    },
    []
  );
  ...
  const onProgressHover = useCallback(
    (e) => {
      const rect = e.currentTarget.getBoundingClientRect();
      const perc = (e.clientX - rect.left) / rect.width;
      const time = perc * (videoRef.current.duration || 1);
      setProgressTooltip({
  ...
  useEffect(() => {
    ...
    const onTime = () => {
      ...
      const b = v.buffered;
      if (b.length) setBufferEnd(b.end(b.length - 1));
    };
      ...
      if (v.canPlayType("application/vnd.apple.mpegurl")) {
        ...
        hls.on(Hls.Events.MANIFEST_PARSED, () => {
          ...
          const opts = [
            ...
            ...lvls.map((l, i) => ({
              label: `${
                l.height ? l.height + "p" : Math.round((l.bitrate || 0) / 1000) + "kb"
              }`,
              index: i,
            })),
          ];
